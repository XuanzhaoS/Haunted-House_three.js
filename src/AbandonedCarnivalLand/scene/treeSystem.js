...
export function addTreesToScene(group, world) {
  ...
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      group.add(tree);
      console.log(
        "Trees added at position (3.8, 0, -6) with left-right arrangement"
      );
  ...
  bush1Positions.forEach((pos, index) => {
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        group.add(bush);
        console.log(
          `Bush1 ${index + 1} added at position (${pos.x}, 0, ${pos.z})`
        );
      },
      (progress) => {
        const percent = (progress.loaded / progress.total) * 100;
        console.log(`Loading bush1 ${index + 1}: ${percent.toFixed(1)}%`);
      },
      (error) => {
  ...
  bushPositions.forEach((pos, index) => {
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        group.add(bush);
        console.log(
          `Bush2 ${index + 1} added at position (${pos.x}, 0, ${pos.z})`
        );
      },
      (progress) => {
        const percent = (progress.loaded / progress.total) * 100;
        console.log(`Loading bush2 ${index + 1}: ${percent.toFixed(1)}%`);
      },
      (error) => {
